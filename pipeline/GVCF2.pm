...
sub vcf {
    my ($tool, $ref_version, $input_gvcf, $out_file_prefix, $out_dir, $region) = @_;
    my ($bed, $cmd);
...
sub multi_sample_gvcf {
    ...
    if($tool =~ /gatk3/i){
        ...
        if ($ref_version =~ /hg19|GRCh37/i){
            $cmd = "$DisGen::general::Resource::_wftool_{gatk4} CombineGVCFs -R $DisGen::general::Resource::_wfdata_{gatk_bundle_hg19}/ucsc.hg19.fasta $input_gvcf_list --dbsnp $DisGen::general::Resource::_wfdata_{gatk_bundle_hg38}/dbsnp_146.hg38.vcf -A StrandOddsRatio -A Coverage -A QualByDepth -A FisherStrand -A MappingQualityRankSumTest -A ReadPosRankSumTest -A RMSMappingQuality -O $out_dir/$out_file_prefix.g.vcf $bed";
